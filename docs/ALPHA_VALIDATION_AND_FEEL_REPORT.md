...
- Routed living-target offhand attacks through `CombatEngine` by building a proper offhand `DamageContext` in [OffhandActionExecutor.java](../src/main/java/com/extremecraft/combat/dualwield/service/OffhandActionExecutor.java).
...
- Updated [CombatEngine.java](../src/main/java/com/extremecraft/combat/CombatEngine.java) so weapon-source-aware contexts can contribute the correct weapon damage instead of always assuming main hand.
- Updated [CombatEngine.java](../src/main/java/com/extremecraft/combat/CombatEngine.java) so `applyDamage(...)` returns the resolved damage result produced during the actual hurt-event pass instead of re-rolling a second preview afterward.
- Added a live client trigger for loadout cycling by registering `CYCLE_LOADOUT` and sending `CycleLoadoutC2S` from [DwClientHooks.java](../src/main/java/com/extremecraft/client/DwClientHooks.java).
- Changed the default class ability key from `R` to `C` in [DwKeybinds.java](../src/main/java/com/extremecraft/client/DwKeybinds.java) to remove the direct conflict with ability-slot 3.
