...
def build_menu_bar(window, callbacks: dict[str, Callable[[], None]]) -> dict[str, QAction]:
    ...
    actions["run_client"] = _action(build_menu, "Run Client", callbacks["run_client"], None, "Run the configured client safely through the session run service")
    actions["run_server"] = _action(build_menu, "Run Server", callbacks["run_server"], None, "Run the configured server safely through the session run service")
    build_menu.addSeparator()
    ...
    build_menu.addSeparator()
    actions["latest_log"] = _action(build_menu, "Latest Log", callbacks["latest_log"], None, "Open the latest runtime log known to the studio")
    actions["clear_logs"] = _action(build_menu, "Clear Logs", callbacks["clear_logs"], None, "Clear in-memory logs and notifications without touching files")
