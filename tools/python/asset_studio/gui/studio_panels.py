...
class GuiStudioPanel(QWidget):
    ...
    def _build_top_actions(self) -> QWidget:
        ...
        doc_row.addWidget(self.document_selector, 1)
        doc_row.addWidget(_button("New", self.new_document, "Create a new GUI document with a root panel."))
        doc_row.addWidget(_button("Open", self.open_document_dialog, "Open a saved GUI document from disk."))
        doc_row.addWidget(_button("Save", self.save_current, "Save the current GUI document."))
        doc_row.addStretch(1)

        primary_row.addWidget(QLabel("Workbench"))
        primary_row.addWidget(_button("Export", self.export_current, "Export the current GUI document to a chosen file path."))
        primary_row.addWidget(_button("Export Runtime", self.export_runtime_current, "Export the current GUI document into the deterministic mod-facing runtime path."))
        primary_row.addWidget(_button("Validate", self.validate_current, "Validate the current GUI definition against the authoritative backend validator."))
        primary_row.addWidget(_button("Preview", self.preview_current, "Refresh the preview payload and selection summary."))
        primary_row.addStretch(1)

        advanced_row.addWidget(QLabel("Advanced"))
        advanced_row.addWidget(_button("Open Runtime", lambda: self._open_related("runtime_export"), "Open the linked runtime export in the main workbench."))
        advanced_row.addWidget(_button("Open Java", lambda: self._open_related("java_target"), "Open the inferred linked Java target in the main workbench."))
        advanced_row.addWidget(_button("Migration Preview", self.preview_migration_current, "Preview schema normalization and diagnostic migration state for the current GUI document."))
    ...
    def _build_center_panel(self) -> QWidget:
        ...
        control_group = QGroupBox("Canvas Controls")
        ...
        align_group = QGroupBox("Layout Tools")
        align = QGridLayout(align_group)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        transform_layout = QGridLayout(transform_row)
        transform_layout.setContentsMargins(0, 0, 0, 0)
        transform_layout.addWidget(QLabel("X"), 0, 0)
        transform_layout.addWidget(self.x_spin, 0, 1)
        transform_layout.addWidget(QLabel("Y"), 0, 2)
        transform_layout.addWidget(self.y_spin, 0, 3)
        transform_layout.addWidget(QLabel("W"), 1, 0)
        transform_layout.addWidget(self.width_spin, 1, 1)
        transform_layout.addWidget(QLabel("H"), 1, 2)
        transform_layout.addWidget(self.height_spin, 1, 3)
...
class ModelStudioPanel(QWidget):
    ...
    def _build_top_actions(self) -> QWidget:
        ...
        doc_row.addWidget(QLabel("Document"))
        doc_row.addWidget(self.document_selector, 1)
        doc_row.addWidget(_button("New", self.new_document, "Create a new model document with a root bone and starter cube."))
        doc_row.addWidget(_button("Open", self.open_document_dialog, "Open a saved model document from disk."))
        doc_row.addWidget(_button("Save", self.save_current, "Save the current model document."))
        doc_row.addWidget(_button("Import", self.open_document_dialog, "Import an existing .model.json document into the workbench."))
        doc_row.addWidget(_button("Export", self.export_current, "Export the current model document to a chosen file path."))
        doc_row.addStretch(1)

        edit_row.addWidget(QLabel("Parts"))
        edit_row.addWidget(_button("Add Part", self.add_part, "Add a new child bone under the current bone or root."))
        edit_row.addWidget(_button("Add Cube", self.add_cube, "Add a cube to the current bone."))
        edit_row.addWidget(_button("Duplicate", self.duplicate_part, "Duplicate the selected cube or bone."))
        edit_row.addWidget(_button("Delete", self.delete_selected, "Delete the selected cube or non-root bone after confirmation."))
        edit_row.addStretch(1)

        preview_row.addWidget(QLabel("Preview / Runtime"))
        preview_row.addWidget(_button("Export Runtime", self.export_runtime_current, "Export the current model document into the deterministic mod-facing runtime path."))
        preview_row.addWidget(_button("Validate", self.validate_current, "Validate the current model document using the authoritative model validator."))
        preview_row.addWidget(_button("Preview", self.preview_current, "Refresh the viewport summary and preview payload."))
        preview_row.addWidget(_button("Open Runtime", lambda: self._open_related("runtime_export"), "Open the linked runtime export in the main workbench."))
        preview_row.addWidget(_button("Open Java", lambda: self._open_related("java_target"), "Open the inferred linked Java target in the main workbench."))
        preview_row.addWidget(_button("Migration Preview", self.preview_migration_current, "Preview schema normalization and diagnostic migration state for the current model document."))
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        helper.setObjectName("panelHelpHint")
        layout.addWidget(helper)
        action_hint = QLabel("Part editing actions are in the top workbench row to keep this outliner focused on selection and structure.")
        action_hint.setWordWrap(True)
        action_hint.setObjectName("panelHelpHint")
    ...
    def _build_center_panel(self) -> QWidget:
        ...
        controls_group = QGroupBox("Viewport Controls")
        ...
        bottom_group = QGroupBox("Viewport Actions")
        bottom = QGridLayout(bottom_group)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        size_layout.addWidget(QLabel("Z"))
        ...
        form.addRow("Size / Pivot", size_row)
