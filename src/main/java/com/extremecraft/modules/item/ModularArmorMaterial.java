...
public enum ModularArmorMaterial implements ArmorMaterial {
    PIONEER("pioneer", 18, map(2, 6, 5, 2), 14, 0.5F, 0.0F),
    ARCSTEEL("arcsteel", 25, map(3, 7, 6, 3), 18, 1.5F, 0.03F),
    TITANIUM("titanium", 30, map(3, 8, 7, 3), 20, 2.0F, 0.05F),
    VOIDWALKER("voidwalker", 37, map(4, 10, 8, 4), 24, 3.5F, 0.10F),
    CELESTIAL("celestial", 42, map(5, 11, 9, 5), 30, 4.5F, 0.15F);
    ...
    ModularArmorMaterial(String id, int durabilityMultiplier, EnumMap<ArmorItem.Type, Integer> defenses, int enchantmentValue, float toughness, float knockbackResistance) {
        this.id = id;
