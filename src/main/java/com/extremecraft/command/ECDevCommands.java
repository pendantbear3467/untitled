...
public final class ECDevCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        root.then(Commands.literal("validate_runtime")
                .requires(src -> src.hasPermission(2))
                .executes(ctx -> ECValidationService.run(ctx.getSource())));

        root.then(Commands.literal("dump_registry")
                .requires(src -> src.hasPermission(2))
                .executes(ctx -> {
        ...
        root.then(Commands.literal("modules")
                .executes(ctx -> {
        ...
        root.then(Commands.literal("reload_data")
                .requires(src -> src.hasPermission(2))
                .executes(ctx -> reloadData(ctx.getSource())));

        root.then(Commands.literal("debug_screen")
                .executes(ctx -> {
                    ServerPlayer player = ctx.getSource().getPlayerOrException();
