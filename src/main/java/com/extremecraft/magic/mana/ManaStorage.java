...
public final class ManaStorage {
    ...
    private double regenAccumulator;

    public ManaStorage(double currentMana, double maxMana, double manaRegen) {
        this.maxMana = Math.max(1.0D, maxMana);
        this.currentMana = clamp(currentMana, 0.0D, this.maxMana);
        this.manaRegen = Math.max(0.0D, manaRegen);
    }
    ...
    public void applyDerived(double nextMaxMana, double nextManaRegen) {
    ...
    public void copyFrom(ManaStorage other) {
