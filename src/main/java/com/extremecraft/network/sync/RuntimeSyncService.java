...
public final class RuntimeSyncService {
    ...
    public static void syncMachineStates(ServerPlayer player) {
        ...
        for (BlockPos pos : BlockPos.betweenClosed(min, max)) {
            ...
            CompoundTag machineState = new CompoundTag();
            if (blockEntity instanceof MachineBlockEntity runtimeMachine) {
                machineState = runtimeMachine.syncTag();
            } else if (blockEntity.getClass().getSimpleName().toLowerCase().contains("machine")) {
                machineState = blockEntity.saveWithoutMetadata();
            }
            ...
            machines.put(pos.toShortString(), machineState);
            included++;
        }
