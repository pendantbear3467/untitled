...
from extremecraft_sdk.definitions.definition_types import AddonSpec, VersionedDependency
...
class DependencyResolver:
    """Resolves graph dependencies and validates compatibility for addon modules."""
    ...
    def _check_platform_compatibility(self, addon: AddonSpec, resolution: DependencyResolution) -> None:
        constraint = addon.compatible_platform_version or addon.compatible_platform
    ...
    def _check_graph_resources(self, addon: AddonSpec, resolution: DependencyResolution) -> None:
        ...
        materials.update(d.id for d in addon.definitions if d.type == "material")
        machines.update(d.id for d in addon.definitions if d.type == "machine")

        for material in addon.dependency_graph.materials:
        ...
        for machine in addon.dependency_graph.machines:
    ...
    def _check_api_dependencies(self, addon: AddonSpec, resolution: DependencyResolution) -> None:
        ...
        for dep in addon.dependency_graph.apis:
            dep_id = dep.id.lower()
            if dep_id in {"extremecraft-api", "api", "extremecraft_api"}:
                if not _version_matches(api_version, dep.version):
                    resolution.conflicts.append(
                        Conflict(
                            kind="api_version_incompatible",
                            identifier=f"api:{api_version}",
                            message=f"Addon requires API version '{dep.version}' but runtime API is '{api_version}'",
                        )
                    )
                continue

            if dep_id in {"extremecraft-protocol", "protocol", "extremecraft_protocol"}:
                if not _version_matches(protocol_version, dep.version):
                    resolution.conflicts.append(
                        Conflict(
                            kind="protocol_version_incompatible",
                            identifier=f"protocol:{protocol_version}",
                            message=(
                                f"Addon requires protocol version '{dep.version}' "
            ...
            resolution.conflicts.append(
                Conflict(
                    ...
                    identifier=f"api:{dep.id}",
                    message=f"Unknown API dependency '{dep.id}' declared in dependency graph",
                )
            )
    ...
    def _resolve_addon_dependency_graph(self, addon: AddonSpec, resolution: DependencyResolution) -> None:
        ...
        queue: deque[tuple[str, VersionedDependency]] = deque((root_key, dep) for dep in addon.dependency_graph.addons)
        ...
        while queue:
            ...
            required_by[dep.id].append(parent_key)
    ...
    def _topological_order(
        ...
        addon_order = [node.removeprefix("addon:") for node in ordered if node.startswith("addon:") and node != root_key]
        return addon_order
    ...
    def _parse_addon_dependencies(self, manifest: dict) -> list[VersionedDependency]:
        ...
        for entry in graph.get("addons", []):
            dep = self._parse_dependency(entry)
            if dep and dep.id not in {d.id for d in dependencies}:
    ...
    def _parse_dependency(self, value: object) -> VersionedDependency | None:
        if isinstance(value, dict):
            dep_id = str(value.get("id", value.get("name", ""))).strip()
            if not dep_id:
                return None
            constraint = str(value.get("version", value.get("constraint", "*"))).strip() or "*"
            return VersionedDependency(id=dep_id, version=constraint)
    ...
    def _load_registry_snapshot(self) -> dict:
    ...
    def _load_runtime_api_versions(self) -> tuple[str, str]:
        source = self.workspace_root.parent / "api" / "src" / "main" / "java" / "com" / "extremecraft" / "api" / "ExtremeCraftApiVersions.java"
        if not source.exists():
            source = self.workspace_root / "api" / "src" / "main" / "java" / "com" / "extremecraft" / "api" / "ExtremeCraftApiVersions.java"
        if not source.exists():
            return ("1", "1")
