...
from compiler.dependency_resolver import DependencyResolution
from extremecraft_sdk.definitions.definition_types import AddonSpec


class DocumentationGenerator:
    """Generates addon and API markdown documentation for compiled modules."""
    ...
    def _addon_doc(self, addon: AddonSpec, resolution: DependencyResolution) -> str:
        ...
        dependencies = "\n".join(
            f"- addon `{dep.id}` ({dep.version})" for dep in addon.dependency_graph.addons
        ) or "- _None_"
        materials = "\n".join(f"- `{material}`" for material in addon.dependency_graph.materials) or "- _None_"
        machines = "\n".join(f"- `{machine}`" for machine in addon.dependency_graph.machines) or "- _None_"
        apis = "\n".join(f"- `{api.id}` ({api.version})" for api in addon.dependency_graph.apis) or "- _None_"
        ...
        return (
            ...
            f"- Compatible platform: `{addon.compatible_platform_version}`\n\n"
            "## Definitions\n"
            f"{definitions}\n\n"
            "## Dependency Graph\n"
            "### Addons\n"
